...
export class ActulizarusuarioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.usuarios.getRequestIdUsuario('https://gymsenapinzon.herokuapp.com/consultaUsuarios/' + this.numeroid, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
  ...
  async onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.usuarios.editarusuario('https://gymsenapinzon.herokuapp.com/actualizar/' + this.numeroid, formularioDeDatos, localStorage.getItem('token')).subscribe(
        (response): any => {
