...
export class EditaranuncioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log("id guzman",this.numeroid)
  ...
  async onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.sistema.editaranuncio('https://gymsenajorge.herokuapp.com/editarAnuncio/'+this.numeroid, formularioDeDatos, localStorage.getItem('token')).subscribe(
        (response): any => {
