...
export class EditarejercicioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.load = false;
    this.client.getRequestIdEjercicios('https://proyectofinalsena.herokuapp.com/consultaEjercicios/' + this.numeroid, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
          console.log(data)
          this.load = true;
  ...
  async onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.client.editarejercicio('https://proyectofinalsena.herokuapp.com/actualizarEjercicio/' + numero.value, formularioDeDatos, localStorage.getItem('token')
      ).subscribe(
        (response): any => {
