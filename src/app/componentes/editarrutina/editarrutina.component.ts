...
export class EditarrutinaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rutina.getRequestIdRutinas(`${environment.BASE_EJERCICIOS}/consultarRutinas` + '/' + this.id, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
  ...
  async onSubmit() {
    ...
    if (this.ejercicios.length >= 1) {
      ...
      this.rutina.editarrutinas(`${environment.BASE_EJERCICIOS}/actualizarRutina/` + numero.value, {
