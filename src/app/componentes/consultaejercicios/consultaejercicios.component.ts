...
export class ConsultaejerciciosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.client.getRequestAllEjercicios(`${environment.BASE_EJERCICIOS}/consultaEjercicios`, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
  ...
  eliminarEjerciciosPorId(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.client.deleteEjercicios(`${environment.BASE_EJERCICIOS}/eliminarEjercicios/` + id, localStorage.getItem('token'))
          .subscribe(
            (data): any => {
