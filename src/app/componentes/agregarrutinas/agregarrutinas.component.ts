...
export class AgregarrutinasComponent implements OnInit {
  ...
  async onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.rutina.registrarrutinas(`${environment.BASE_EJERCICIOS}/registrarRutinas`, {
  ...
  Ejercicios() {
    this.rutina.getReporteAllEjercicios(`${environment.BASE_EJERCICIOS}/ejercicios`, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
