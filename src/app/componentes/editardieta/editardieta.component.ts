...
export class EditardietaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dieta.getRequestIdDietas(`${environment.BASE_EJERCICIOS}/consultarDietas/` + this.numeroid, localStorage.getItem('token'))
      .subscribe(
        (data): any => {
  ...
  async onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.dieta.editarDietas(`${environment.BASE_EJERCICIOS}/actualizarDietas/` + numero.value, {
